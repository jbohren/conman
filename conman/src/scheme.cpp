...
bool Scheme::addConnection(const std::string &source_cmp, const std::string &source_port, const std::string &target_cmp , const std::string &target_port, bool feedback) {
  if(!this->hasPeer(source_cmp)) {
    RTT::log(RTT::Error) << "No such component : " << source_cmp << RTT::endlog();
    return false;
  }

  if(!this->hasPeer(target_cmp)) {
    RTT::log(RTT::Error) << "No such component : " << target_cmp << RTT::endlog();
    return false;
  }
  
  RTT::TaskContext *src = this->getPeer(source_cmp);
  RTT::TaskContext *target = this->getPeer(target_cmp);
  
  RTT::base::PortInterface *sp, *tp;
  
  sp = src->getPort(source_port);
  tp = target->getPort(target_port);
  
  if(!sp) {
    RTT::log(RTT::Error) << "No such port '" << source_port << "' in component '" << source_cmp << "'" << RTT::endlog();
    return false;
  }
  
  if(!tp) {
    RTT::log(RTT::Error) << "No such port '" << target_port << "' in component '" << target_cmp << "'" << RTT::endlog();
    return false;
  }
  
  if(sp->connectTo(tp, RTT::ConnPolicy(RTT::ConnPolicy::DATA, RTT::ConnPolicy::UNSYNC))) {
    RTT::log(RTT::Info)<< "Connected Port " << source_port << " to  "<< target_port <<"." << RTT::endlog();
  } else {
    RTT::log(RTT::Error)<< "Failed to connect Port " << source_port << " to "<< target_port <<"." << RTT::endlog();
    return false;
  }
...
//    sink_name = sink_port->getInterface()->getOwner()->getName();
  ...
  if( flow_vertex_maps_.find(src) != flow_vertex_maps_.end() && 
      flow_vertex_maps_.find(target) != flow_vertex_maps_.end()) 
  {
    // Get the existing edges between these two blocks
    // NOTE: Using out_edges instead of edge_range because edge_range is buggy
    graph::BlockOutEdgeIterator existing_edge_it, existing_edge_end;
    boost::tie(existing_edge_it, existing_edge_end) = 
      boost::out_edges(flow_vertex_maps_[src], flow_graphs_);
    ...
    for(; existing_edge_it != existing_edge_end; ++existing_edge_it) {
      if( flow_graphs_[*existing_edge_it]->source_port == sp &&
          flow_graphs_[*existing_edge_it]->sink_port == tp) {
        // The edge exists
        edge_exists = true;
        break;
      }
    ...
    if(!edge_exists) {
      ...
      graph::EdgeProperties::Ptr edge_props = boost::make_shared<graph::EdgeProperties>();
      edge_props->connected = true;
      edge_props->source_port = sp;
      edge_props->sink_port = tp;

      // Add the edge to the graph
      boost::add_edge(flow_vertex_maps_[src], flow_vertex_maps_[target], edge_props, flow_graphs_);

      RTT::log(RTT::Debug) << "Created edge "
        <<source_cmp<<"."<<sp->getName()<<" --> "
        <<target_cmp<<"."<<tp->getName()<< RTT::endlog();
      
      regenerateGraph();
      
      return true;
    } else {
//      RTT::log(RTT::Debug) << "Existis edge "
...
bool Scheme::addBlock(RTT::TaskContext *new_block)
{
  ...
  // Connect the block in the appropriate ports
  bool success = true;
  ...
  for(BlockOrdering::iterator it = causal_ordering_.begin();
      it != causal_ordering_.end();
      ++it) 
  {
    RTT::log(RTT::Info) << flow_graphs_[*it]->block->getName() << ", ";
  }
    ...
    conman::graph::VertexProperties::Ptr new_vertex)
{
  ...
  flow_vertex_maps_[new_block] = boost::add_vertex(new_vertex, flow_graphs_);

  RTT::log(RTT::Debug) << "Created vertex: "<< new_vertex->index << " (" <<
    flow_vertex_maps_[new_block]<<")" << RTT::endlog();
    ...
    RTT::TaskContext *block)
{
  ...
  BlockGraph &flow_graph = flow_graphs_;
  BlockVertexMap &flow_vertex_maps_ = flow_vertex_maps_;

  // Check if the block is in this layer
  if(flow_vertex_maps_.find(block) != flow_vertex_maps_.end()) {
    // Get the vertex properties pointer
    VertexProperties::Ptr vertex = flow_graph[flow_vertex_maps_[block]];
    ...
    conman::graph::VertexProperties::Ptr vertex)
{
  ...
  if(flow_vertex_maps_.find(vertex->block) == flow_vertex_maps_.end()) {
    return true;
  }

  // Remove the edges connected to this vertex 
  boost::clear_vertex(flow_vertex_maps_[vertex->block], flow_graph);
  // Remove the vertex 
  boost::remove_vertex(flow_vertex_maps_[vertex->block], flow_graph);
  // Remove the vertex from the map
  flow_vertex_maps_.erase(vertex->block);
...
bool Scheme::regenerateGraph()
{
  ...
  // Get references to the graph structures
  BlockGraph &flow_graph = flow_graphs_;
  BlockOrdering &ordering = causal_ordering_;
  BlockVertexMap &flow_vertex_maps_ = flow_vertex_maps_;

  bool topology_modified = true;

  if(topology_modified) {
    // Recompute topological sort (and require that this is still a DAG)
    try {
      ...
      // Complain
      RTT::log(RTT::Error)
        << "Cannot regenerate topological ordering in conman scheme. because: " << ex.what() << RTT::endlog();
...
void Scheme::computeConflicts(RTT::TaskContext *block)
{
  ...
  boost::graph_traits<BlockGraph>::in_edge_iterator in_edge_it, in_edge_end;
    ...
    boost::tie(out_edge_it, out_edge_end) = boost::out_edges(flow_vertex_maps_[block], flow_graphs_);
    ...
    for(;out_edge_it != out_edge_end; ++out_edge_it) {
      ...
      EdgeProperties::Ptr edge = flow_graphs_[*out_edge_it];

      // Get a reference to the vertex properties of the sink for convenience
      BlockVertexDescriptor sink_vertex_descriptor = boost::target(*out_edge_it, flow_graphs_);
      VertexProperties::Ptr sink_vertex = flow_graphs_[sink_vertex_descriptor];
      ...
      if(mode == conman::Exclusivity::EXCLUSIVE) {
        ...
        boost::tie(in_edge_it, in_edge_end) = boost::in_edges(sink_vertex_descriptor, flow_graphs_);
        ...
        for(;in_edge_it != in_edge_end; ++in_edge_it) {
          ...
          if(flow_graphs_[*in_edge_it]->sink_port == edge->sink_port) {
            // Add conflict between the seed block and the source block for this edge
            VertexProperties::Ptr conflicting_vertex = flow_graphs_[boost::source(*in_edge_it,flow_graphs_)];
...
void Scheme::updateHook() 
{
      ...
      ++it)
  {
    ...
    VertexProperties::Ptr block_vertex = flow_graphs_[*it];
