...
ROSInterfaceService::ROSInterfaceService(RTT::TaskContext* owner)
  ...
  if (!scheme) {
    std::string err_text =
        "Attmpted to load the Conman ROS interface on a component which isn't a scheme!";
    RTT::log(RTT::Error) << err_text << RTT::endlog();
    throw std::runtime_error(err_text);
  }

  // Connect operation callers
  RTT::log(RTT::Debug) << "Connecting conamn_ros operation callers..."
                       << RTT::endlog();
  ...
  RTT::log(RTT::Debug) << "Creating ros_control service servers..."
                       << RTT::endlog();
  roscontrol = owner->provides("roscontrol");
  roscontrol->addOperation("listControllerTypes",
                           &ROSInterfaceService::listControllerTypesCB, this);
  roscontrol->addOperation("listControllers",
                           &ROSInterfaceService::listControllersCB, this);
  roscontrol->addOperation("loadController",
                           &ROSInterfaceService::loadControllerCB, this);
  roscontrol->addOperation("reloadControllerLibraries",
                           &ROSInterfaceService::reloadControllerLibrariesCB,
                           this);
  roscontrol->addOperation("switchController",
                           &ROSInterfaceService::switchControllerCB, this);
  roscontrol->addOperation("unloadController",
                           &ROSInterfaceService::unloadControllerCB, this);
  ...
  RTT::log(RTT::Debug) << "Getting rtt_roscomm service service..."
                       << RTT::endlog();
  rosservice = owner->getProvider<rtt_rosservice::ROSService>("rosservice");

  RTT::log(RTT::Debug) << "Connecting ros_control service servers..."
                       << RTT::endlog();
  ...
  // Actions
  get_blocks_action_server_.addPorts(
      this->provides("get_blocks_action"), true,
      "~" + this->getOwner()->getName() + "/get_blocks_action/");
  get_blocks_action_server_.registerGoalCallback(
      boost::bind(&ROSInterfaceService::get_blocks_goal_cb, this, _1));
  get_blocks_action_server_.start();

  set_blocks_action_server_.addPorts(
      this->provides("set_blocks_action"), true,
      "~" + this->getOwner()->getName() + "/set_blocks_action/");
  set_blocks_action_server_.registerGoalCallback(
      boost::bind(&ROSInterfaceService::set_blocks_goal_cb, this, _1));
  set_blocks_action_server_.start();

  // Introspection
  introspection = owner->provides("introspection");
  introspection->addOperation("broadcastGraph",
                              &ROSInterfaceService::broadcastGraph, this,
                              RTT::ClientThread).doc(
      "Broadcast a graphviz representation of the scheme and its members.");
  introspection->addPort("dotcode_out", dotcode_out_);
  dotcode_out_.createStream(
      rtt_roscomm::topic("~" + owner->getName() + "/dotcode"));
...
bool ROSInterfaceService::switchControllerCB(
  ...
  RTT::log(RTT::Debug) << "Handling ros_control switch controllers request..."
                       << RTT::endlog();
...
void ROSInterfaceService::get_blocks_goal_cb(
    actionlib::ServerGoalHandle<conman_msgs::GetBlocksAction> gh) {
...
void ROSInterfaceService::set_blocks_goal_cb(
    actionlib::ServerGoalHandle<conman_msgs::SetBlocksAction> gh) {
  ...
  bool success = scheme->switchBlocks(goal->disable, goal->enable, goal->strict,
                                      goal->force);
...
void ROSInterfaceService::broadcastGraph() {
  ...
  for (std::vector<conman::BlockDescription>::const_iterator block_it =
      block_descriptions.begin(); block_it != block_descriptions.end();
      ++block_it) {
    std::ostringstream &oss = main_stream;
    oss << block_it->name + "[shape=record, label=\"\\N|{{";

    for (std::vector<std::string>::const_iterator port_name_it = block_it
        ->input_ports.begin(); port_name_it != block_it->input_ports.end();
        ++port_name_it) {
      oss << "<" << sanitize(*port_name_it) << ">" << *port_name_it << "|";
    }
    oss << "}| |{";
    for (std::vector<std::string>::const_iterator port_name_it = block_it
        ->output_ports.begin(); port_name_it != block_it->output_ports.end();
        ++port_name_it) {
      oss << "<" << sanitize(*port_name_it) << ">" << *port_name_it << "|";
    }
    ...
    RTT::Seconds pavg = hook->getPeriodAvg(), pmin = hook->getPeriodMin(),
        pmax = hook->getPeriodMax(), pvar = hook->getPeriodVar();
    RTT::Seconds davg = hook->getDurationAvg(), dmin = hook->getDurationMin(),
        dmax = hook->getDurationMax(), dvar = hook->getDurationVar();
    ...
    oss << boost::str(
        boost::format(
            "|{{%1.2e|%1.2e}|{%1.2e +/- %1.1e|%1.2e +/- %1.1e (%3.1f%%)}|{%1.2e|%1.2e}}")
            % pmin % dmin % pavg % pvar % davg % dvar % (100.0 * fraction)
            % pmax % dmax);
    ...
    if (task->isRunning()) {
      oss << " fillcolor=\""
          << 0.66 * (1.0 - std::min(std::max(fraction, 0.0), 1.0))
          << " 0.6 0.8 \"";
      oss << " color=\""
          << 0.66 * (1.0 - std::min(std::max(fraction, 0.0), 1.0))
          << " 0.6 0.3 \"";
  ...
  for (std::vector<conman::ConnectionDescription>::const_iterator conn_it =
      conn_descriptions.begin(); conn_it != conn_descriptions.end();
      ++conn_it) {
    std::ostringstream &oss = main_stream;
    oss << conn_it->source << ":" << sanitize(conn_it->source_port) << " -> "
        << conn_it->sink << ":" << sanitize(conn_it->sink_port) << ";";
  }
